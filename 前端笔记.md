...
11. 使用 float 之后，会使元素脱离文档流，无法使用margin：0  auto；使元素居中(img是内联元素不能用这个居中，需要转换为块级元素。或者父元素加上text-align：center)

12. input 标签里设置 placeholder 可以提示信息  

13. padding 设置为百分比是参照父元素的宽度来给定的。可以用来
...
15. max-height（width）如果图片的高（宽）超过设置高度，则将高（宽）缩放到设定的最大值。

16. 两个相邻块状元素，上面一个设置了阴影，下面一个设置了背景颜色，则阴影会被遮挡。需要在上一个元素设置position：relative       z-idnex 仅在定位元素上生效

17. class1和class2**同级相邻**，若要 class1：hover 控制 class2 可以用加号   class1：hover  +  class2 {}

18.    标签：last-child {}    需要同时满足两个条件才能选中
...
1.onkeyperss事件触发只对有字符按键生效，无字符按键如：tab、caps、shift等按下不触发事件

2.事件属性名都为小写

3.被查找字符串.indexOf(“字符串”)   返回查找第一次出现的位置 ,注意0，如果找不到返回-1

4.[].foreach()  方法在 ie8 中不支持。
...
12.在 js 中，函数是一种特殊的对象，所以可以对函数添加属性。下面代码可以通过编译。
...
14.除基础对象外的所有对象都有原型，基础对象的原型为 null
